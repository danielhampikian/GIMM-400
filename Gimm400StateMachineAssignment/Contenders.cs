...
public abstract class Contender : MonoBehaviour, IAttackAI, IProjectile
{
    ...
    public abstract float _timeRemaining { get; set; }
    ...
    public float _projSpeed { get; set; }
    ...
    public bool CheckValue() //checks to see if ratio's work
    {
        ...
        {
            Debug.LogException(new System.Exception("Health needs to be in between 1 and .1"));
            return false;
        }

        if (_damage > .1 || _damage < 1)
        {
            Debug.LogException(new System.Exception("Damage needs to be in between 1 and .1"));
            return false;
        }
        if (_Speed > .1 || _Speed < 1)
        {
            Debug.LogException(new System.Exception("Speed needs to be in between 1 and .1"));
            return false;
        }
        ...
        bool check = CheckRatios();

         //set ratios and multpliers:
    ...
    public virtual void TakeDamage(float amt) //takes the damage from the projectile
    {
        _health -= amt;
        if (_health < 0)
        {
            onGameOver?.Invoke(); //This will call onGameover as an event that other classes can listen to and be notified of like GameManager
